<template>
  <div>
    ...
    <ul>
      ...
      <li>Heap(used/max): {{ readableDataSize(data.metrics.heapMemoryUsed) }}/{{ readableDataSize(data.metrics.heapMemoryMaximum) }}</li>
      <li>OffHeap(used/commited/max): {{ readableDataSize(data.metrics.nonHeapMemoryUsed) }}/{{ readableDataSize(data.metrics.nonHeapMemoryCommitted) }}/{{ readableDataSize(data.metrics.nonHeapMemoryMaximum) }}</li>
    </ul>
    <h2>Nodes</h2>
    <v-data-table
