...
class AggregatorPlanner(val binding: Map<String, Aggregator>) {
    fun plan(planner: Planner, ast: AggregatorCall, ctx: PlannerContext): InstantNode {
    ...
    private fun planMappingAggregation(params: List<PlanNode>, ast: AggregatorCall, ctx: PlannerContext): InstantNode {
        if (params.size != 1) {
        ...
        val mp = param.metric as? FixedMetric
                ?: return ColumnMapAggregatorDynamicNode(param, ast.agg.name, ast.groups)

        val group = ast.groups
        return if (group == null) {
            ColumnMapAggregatorNode(FixedMetric(listOf(Metric.empty)), param, ast.agg.name, null)
        } else {
